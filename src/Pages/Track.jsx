...
import { LuClock3, LuMinusCircle, LuRadio } from "react-icons/lu";
...
const Track = () => {
  ...
  const song = findById(songs, id);
  console.log(song);

  const likeSong = () => {
    axios
      .post(
        `${server}/api/v1/playlist/like/song`,
        {
          id: song.id,
          title: song.title,
          artist: song.artist,
      ...
      <div className="lay relative w-[62rem] h-[88vh] bg-gradient-to-b from-[#202020] to-[#121212] overflow-x-hidden overflow-y-scroll ml rounded-md">
        ...
        <div className="w-full">
          <div className="w-full px-4 h-[30vh] bg-gradient-to-b pt-20 from-[#525252] to-[#2C2C2C]">
            <div className="flex gap-4 items-end h-full py-4">
              <div className="w-32 h-32 rounded-md bg-[#282828] overflow-hidden">
                <img src={song?.img} className="w-full h-full" />
              </div>
              <div>
                ...
                <button onClick={() => setOpen(true)}>
                  <h1 className="text-5xl tracking-tight font-extrabold mb-3">
                    {song?.title}
                  </h1>
                </button>
          ...
          <div className="w-full relative py-2 m-5 flex justify- gap-5 items-center">
            {menu && (
              <div className="w-[17rem] p-1 bg-[#282828] shadow rounded-md absolute z-[999] top-12 left-24 text-zinc-300 flex flex-col">
            ...
            {user?.likedSongs?.find((song) => song?.id === song.id) ? (
              <button
                onClick={likeSong}
                className="text-xs p-1 rounded-full bg-green-600 text-black"
              >
              ...
              <button
                onClick={likeSong}
                className="play p-2 border text-sm rounded-full"
              >
                <FaPlus />
              </button>
            )}
